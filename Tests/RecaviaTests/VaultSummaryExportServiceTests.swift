...
@testable import Recavia
    ...
    struct VaultSummaryExportServiceTests {
        ...
        func exportSummaryBundleWritesSummaryTranscriptAndScreenshots() async throws {
            ...
            #expect(FileManager.default.fileExists(atPath: vaultURL.appendingPathComponent("_recavia/transcripts/\(meetingId.uuidString).md").path))
            #expect(
                FileManager.default.fileExists(
                    atPath: vaultURL.appendingPathComponent("_recavia/screenshots/\(screenshot.id.uuidString).png").path
                )
            )
        }
