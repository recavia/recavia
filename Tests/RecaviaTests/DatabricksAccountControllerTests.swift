    ...
    struct DatabricksAccountControllerTests {
        ...
        func validProfileConfiguresCodexAndLoadsModels() async {
            let rootURL = FileManager.default.temporaryDirectory
                .appending(path: "recavia-databricks-account-\(UUID().uuidString)", directoryHint: .isDirectory)
        ...
        func cancelledProfileLoadDoesNotApplyConfiguration() async throws {
            let rootURL = FileManager.default.temporaryDirectory
                .appending(path: "recavia-databricks-cancellation-\(UUID().uuidString)", directoryHint: .isDirectory)
            defer { try? FileManager.default.removeItem(at: rootURL) }
