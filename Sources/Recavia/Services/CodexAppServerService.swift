...
actor CodexAppServerService {
    ...
    private let logger = Logger(subsystem: "com.recavia", category: "CodexAppServer")
    ...
    nonisolated static func summaryThreadConfig(
        ...
        if let recaviaMCP {
            guard !recaviaMCP.allowedMeetingIDs.isEmpty else {
                throw CodexAppServerError.invalidProtocolResponse
            }
            var servers = config["mcp_servers"]?.objectValue ?? [:]
            servers["recavia"] = recaviaMCPServer(
                executableURL: recaviaMCP.executableURL,
                vaultID: recaviaMCP.vaultID,
                allowedMeetingIDs: recaviaMCP.allowedMeetingIDs
            )
    ...
    private nonisolated static func recaviaMCPServer(
        executableURL: URL,
        vaultID: UUID,
...
private extension CodexAppServerService {
    ...
    private func performGeneration(
        ...
        let temporaryDirectory = FileManager.default.temporaryDirectory
            .appending(path: "recavia-codex-\(generationID.uuidString)", directoryHint: .isDirectory)
    ...
    private nonisolated static func summaryToolInstruction(allowsRecaviaMCP: Bool) -> String {
        if allowsRecaviaMCP {
            return "\nYou may call only the Recavia get_meeting tool. Do not call any other tool. Return only the requested JSON."
        }
