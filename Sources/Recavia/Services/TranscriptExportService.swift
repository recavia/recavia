...
enum TranscriptExportService {
    static func transcriptsDirectoryURL(in vaultURL: URL) -> URL {
        vaultURL
            .appendingPathComponent("_recavia", isDirectory: true)
    ...
    /// 文字起こしを `<vault>/_recavia/transcripts/{meetingId}.md` に書き出す。
    /// ファイルが既に存在する場合は上書きする。
    /// - Returns: vault 相対パス（例: `_recavia/transcripts/XXXXXXXX-....md`）
    static func exportTranscript(
        ...
        let relativePath = "_recavia/transcripts/\(meetingId.uuidString).md"
