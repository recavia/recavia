...
この文書は、Recavia の `AGENTS.md` を更新するときの判断基準を記録する。実行時に常に必要なルールだけを `AGENTS.md` に置き、背景説明や保守手順はここへ分離する。

## 基本方針

GPT-5.6 には、手順を細かく固定するより、成果、重要な制約、利用できる根拠、完了条件を明確に伝える。既に安定している一般的な振る舞いは説明せず、Recavia 固有の判断を変える情報を優先する。
...
- `Sources/Recavia/AGENTS.md`: アプリ固有の所有関係、並行処理、UI とローカライズ
- `Sources/Recavia/Database/AGENTS.md`: データ保全とマイグレーション
- `Tests/RecaviaTests/AGENTS.md`: テストの隔離、実装規約、実行結果の判定
