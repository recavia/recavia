...
Recavia のバッチ録音は、マイクとシステム音声を音源ごとの CAF に追記し、録音中は
`*.partial.caf`、正常終了後は `*.caf` として管理している。音声本体は
`~/Library/Application Support/Recavia/BatchAudio`、セッション、ファイル、range のメタデータは
SQLite に保存する。
...
- `Application Support/Recavia` と録音ファイルに、Recavia としての POSIX permission の契約がない。
...
- Recavia 内の競合、復旧、再試行、削除処理が、完成済みの録音を誤って失わせないこと。
...
| 事象 | 方針 |
|------|------|
| Recavia の crash、強制終了、OS crash、電源断 | 完成済みセグメントを不変にし、active / finalizing の partial だけを復旧対象にする |
| 複数の Recavia プロセス、旧プロセスの残存、起動時復旧との競合 | 単一インスタンス、process-wide lock、session lease の三層で mutation の所有権を排他する |
...
`removeItem` が成功しても、Recavia は「すべての物理コピーが復元不能になった」とは表示しない。
...
Recavia は製品仕様として単一の interactive instance だけを許可し、`Info.plist` に
`LSMultipleInstancesProhibited = true` を設定する。ただし、これは Launch Services を経由しない `swift run Recavia`、実行ファイルの直接起動、
...
Application Support の Recavia 管理 root に process-wide `.process.lock` を置き、DB の read-write open、startup recovery、録音開始より前に
non-blocking の排他的 `flock` を取得する。取得できない二つ目の process は録音データと DB を変更せず、既に Recavia が起動中であることを
通知して終了する。
...
`flock` を取得する。process-wide lock は Recavia 全体の同時 mutation を防ぎ、session lease は crash recovery で個別セッションの所有権を
...
```text
Application Support/Recavia/BatchAudio/<meeting-id>/<session-id>/
├── .lease
...
- 正常に `ready` へ到達した segment は、明示的な retention / deletion 遷移なしに Recavia が変更・削除しない。
...
- 通常の二重起動が拒否され、`swift run Recavia` と署名済み app 等が競合した場合も process-wide lock を取得できない側が DB と音声を変更しない。
